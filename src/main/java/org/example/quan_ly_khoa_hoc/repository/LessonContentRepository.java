...
public class LessonContentRepository implements ILessonContentRepository {
    ...
    public List<LessonContentDTO> findByLessonId(int lessonId) {
        ...
        String sql = "SELECT content_id, lesson_id, content_type, content_data FROM lesson_contents WHERE lesson_id = ? ORDER BY content_id ASC";
    ...
    public LessonContentDTO findById(int contentId) {
        String sql = "SELECT content_id, lesson_id, content_type, content_data FROM lesson_contents WHERE content_id = ?";
        try (Connection conn = DatabaseUtil.getConnectDB();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public LessonContentDTO save(LessonContentDTO lessonContentDTO) {
        ...
        try (Connection conn = DatabaseUtil.getConnectDB();
            ...
            return lessonContentDTO; // Return the DTO with generated ID
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public void update(LessonContentDTO lessonContentDTO) {
        String sql = "UPDATE lesson_contents SET content_type = ?, content_data = ? WHERE content_id = ?";
        try (Connection conn = DatabaseUtil.getConnectDB();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, lessonContentDTO.getContentType());
            ps.setString(2, lessonContentDTO.getContentData());
            ps.setInt(3, lessonContentDTO.getContentId());
            ps.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
